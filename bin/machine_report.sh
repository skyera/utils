...
host_name_upper=$(hostname -s 2>/dev/null | tr '[:lower:]' '[:upper:]')
report_title="${host_name_upper:-SERVER} - SYSTEM REPORT"
...
if command -v lastlog &>/dev/null; then
    ...
    # Grab previous login line from wtmp
    last_line=$(last -n 2 -R "$USER" 2>/dev/null | sed -n '2p')
    if [ -n "$last_line" ]; then
        last_login_time=$(echo "$last_line" | awk '{print $3, $4, $5, $6}')
    else
...
PRINT_CENTERED_DATA "MACHINE STATUS"
PRINT_DIVIDER "top"
